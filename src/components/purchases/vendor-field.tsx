...
export function VendorField({
  ...
  const isBusy = disabled || adding || deletingId !== null;
  ...
  return (
    <>
      <ConfirmModal
        ...
        message={
          pendingDelete
            ? `Remove "${pendingDelete.name}" from your vendor list? If it has purchase history, delete those purchases from History first.`
            : ""
        }
      ...
      <div ref={containerRef} className="space-y-2">
          ...
          <div className="overflow-hidden rounded-xl border border-[var(--input-border)] bg-[var(--card)] shadow-lg">
            ...
            >
              {filteredVendors.map((vendor) => (
                <li key={vendor.id} className="border-b border-[var(--card-border)] last:border-b-0">
                  <button
                    type="button"
                    onClick={() => handleSelectExisting(vendor)}
                    className="min-h-12 w-full px-4 py-3 text-left text-sm text-[var(--foreground)] transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/80"
                  >
                    {vendor.name}
                  </button>
                </li>
        ...
        {selectedVendor && (
          <div className="animate-fade-in rounded-xl border border-emerald-200 bg-emerald-50/50 dark:border-emerald-900/50 dark:bg-emerald-950/30">
            <div className="flex items-center justify-between gap-2 px-3 py-2.5">
              <p className="flex min-w-0 items-center gap-2 text-sm font-medium text-emerald-700 dark:text-emerald-400">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-xs text-white dark:bg-emerald-500">
                  ✓
                </span>
              ...
              <button
                ...
                className="min-h-9 shrink-0 rounded-lg border border-[var(--input-border)] bg-[var(--card)] px-3 text-xs font-semibold text-[var(--foreground)] hover:bg-gray-50 disabled:opacity-50 dark:hover:bg-gray-800"
              >
                Change
              </button>
            </div>
            <button
              type="button"
              disabled={isBusy}
              onClick={() => setShowDeleteOptions((current) => !current)}
              className="flex w-full min-h-10 items-center justify-between border-t border-emerald-200/80 px-3 text-xs font-semibold text-[var(--muted)] dark:border-emerald-900/50"
            >
              <span>Vendor options</span>
              <span
                className={`transition-transform ${showDeleteOptions ? "rotate-180" : ""}`}
                aria-hidden
              >
                ▼
              </span>
            ...
            {showDeleteOptions && (
              <div className="border-t border-emerald-200/80 px-3 py-2 dark:border-emerald-900/50">
                <button
                  type="button"
                  disabled={isBusy}
                  onClick={() => {
                    setShowDeleteOptions(false);
                    setPendingDelete(selectedVendor);
                  }}
                  className="min-h-10 w-full rounded-lg border border-red-200 px-3 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:border-red-900/50 dark:text-red-400 dark:hover:bg-red-950/40"
                >
                  Delete vendor
                </button>
              </div>
